  ...
  Datepicker.prototype = {
    ...
    parse2DateObj:function(sDate,sSign){
        ...
        if (dateInfoArray.length ==3) {
            ...
            return {status:1,dateObj:date};
        } else {
            if(sDate==""){
                return {status:1,dateObj:new Date()};
            }
            else{
                return {status:0,dateObj:new Date()};
            }
        }
    },
    /**
    ...
    getMonth:function(dateObj){
        ...
        for (var i = 1; i < startweek; i++) {
            ...
            html += "<td class='muted p_month'>"+tmpI + (enableLunar?"<br/>" + this.getSolarDay(year, month - 1, tmpI,showmon):"")+"</td>";
            index++;
        }
        var sn=0;
        var sStyle="";
        for (var i = 1; i <= days; i++) {
            ...
            if (index % 7 == 0) {
                ...
                weekSn++;
                html += "<td class='text-error'"+sStyle+">" + i + (enableLunar?"<br/>" + this.getSolarDay(year, month, i,showmon):"") + "</td>";
                sn++;
            }
            else{
                ...
                if(index-sn*7==6){
                    ...
                    html += "<td"+sStyle+">" + i + (enableLunar?"<br/>" + this.getSolarDay(year, month, i,showmon):"") + "</td>";
                }
            }
            index++;
        }
        ...
        for (var i = 0; i < 14; i++) {
            ...
            tmpI=i+1;
            html += "<td class='muted n_month'>"+tmpI + (enableLunar?"<br/>" + this.getSolarDay(year, month + 1, tmpI,showmon):"")+"</td>";
            index++;
        }
        html += "</tr>";
        return html;
    ...
    getFullMonth:function(dateObj){
        ...
        sHtml="<table class='table-condensed tc'><thead>"+
            (this.options.Lunar.enable?"<tr><td colspan=8>"+this.solarDay2(year,month,day,1)+"</td></tr>":"")+
            "<tr><td colspan='8'><a class='mr15' href='#'><i class='icon-backward'></i></a><a class='mr15' href='#'><i class='icon-chevron-left'></i></a>"+year+sYear+(month+1)+sMonth+"<a class='ml15' href='#'><i class='icon-chevron-right'></i></a><a class='ml15' href='#'><i class='icon-forward'></i></a></td></tr>"+
    ...
    getSolarDay:function(year, month, day, showmon) {
        ...
        return this.cDay(lDObj.month, lDObj.day, showmon);
    },
    /**
     * 显示农历年月日
     * @param  {number} year  年
    ...
    solarDay2:function(year, month, day, showmon) {
        ...
        //农历BB'+(cld[d].isLeap?'闰 ':' ')+cld[d].lMonth+' 月 '+cld[d].lDay+' 日 
        var tt = '农历' + this.cDay(lDObj.month, lDObj.day, showmon);
        return (cl + tt + '</font>');
    },
    /**
     * 显示节假日和节气信息
