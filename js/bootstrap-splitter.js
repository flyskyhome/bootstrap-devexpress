	...
	Splitter.prototype = {
		...
		create: function(options) {
			...
			var bar = $('<div></div>').insertAfter(A).addClass(opts.barClass).addClass(opts.barStateClass).append(focuser).attr({
				...
				"z-index": "100"
			}).bind("mousedown" + opts.eventNamespace,{eventObj:opts.dragEnd},startSplitMouse).bind("mouseover" + opts.eventNamespace, function() {
...
//					splitter.triggerHandler(pw ? "dock" : "undock");
					...
					if(opts.dockPane.first().css("display")=="none"){
						opts.dockPane.first().css("display","");
					...
					//opts.dockPane == A?A.css("display","none"):B.css("display","none");
				}).bind("undock" + opts.eventNamespace, function() {
					//					var pw = opts.dockPane[0][opts.pxSplit];
					//var pw = opts.dockPane[0][opts.pxSplit] - getPanelBorderSize(opts.type, opts.dockPane[0]);
					var pw=A[0].clientWidth>0 && splitter._DA-bar._DA>A[0][opts.pxSplit];
